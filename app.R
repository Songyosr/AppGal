...
# m <- basemap
# 
# lapply(2016:2020, function(x,Dt){
#   m <<- .GlobalEnv$m %>% 
#     addPolygons(data = Dt[Dt$y == year,],
#                 fillColor = ~pal(Stunt),
#                 weight = 1,
#                 opacity = 0.4,
#                 color = "white",
...
# pal(tmp$ind)
# class(tmp$ind %>% as.numeric())


# st_geometry(tmp[,indi]) <- NULL
# test <- `st_geometry<-`(tmp[,indi], NULL) %>% unlist(use.names = F)
# 
# ?unlist
